...
#years we want to sample, set to something other than
#c(1:max.year) if we want to skip certain years
sampling.years <- rep(list(c(1:max.year),
                           seq(1,max.year,by=2),
                           seq(1,max.year,by=4)),
                           100)[[jobid]]
...
for(j in 1:length(beta1)){ # loop over beta1's
  ...
  for(sim in 1:num.sim){
    ...
    # removed data for un-sampled years
    dat2 <- dat2[which(dat2$time %in% sampling.years),]
    
    # Load data
    data <- list(y = as.numeric(dat2$measurement), n = dim(dat2)[1], site = dat2$sites, year = match(dat2$time,sort(unique(dat2$time))), yearX = dat2$z_year,
